...
| Pro recurring/scheduled SOL and devnet USDC payments | RecurringSchedule/RecurringTokenSchedule PDAs and Clear Wallet program | Product-wired onchain schedule state. Configuration/revocation require typed threshold governance; one due payment can be executed permissionlessly at a time. USDC binds Circle's devnet mint and exact source/destination token accounts. It supports recipient and allowed-hours policy controls; SOL-denominated numeric policy fields are rejected until CSP2 asset-scoped ledgers land. An external caller is still required for each due execution. |
