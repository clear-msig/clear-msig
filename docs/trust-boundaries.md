...
| Execution library / CLI adapter | Refuse execution, fail after a partial step, or submit the wrong transaction attempt. | Make a destination transaction with fields different from the approved preimage. | Program verifies the typed action; destination chain verifies the signature; signed MessageApproval state supports resume. | The shared library still contains CLI-shaped parsing and blocking infrastructure adapters; typed command inputs and cancellable async ports should become narrower core interfaces. |
