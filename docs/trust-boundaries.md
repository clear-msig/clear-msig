...
| Execution library / CLI adapter | Refuse execution, fail after a partial step, or submit the wrong transaction attempt. | Make a destination transaction with fields different from the approved preimage. | Typed proposal lifecycle and execution routes construct bounded domain values with mutually exclusive signer contexts; remaining adapter arguments are validated into an opaque typed request. Program and destination chains verify committed bytes. | Legacy proposal, wallet, and intent routes still build adapter arguments, and infrastructure adapters remain blocking. |
