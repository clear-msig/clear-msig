...
| Execution library / CLI adapter | Refuse execution, fail after a partial step, or submit the wrong transaction attempt. | Make a destination transaction with fields different from the approved preimage. | Typed proposal execution routes construct bounded domain values directly; remaining adapter arguments are validated into an opaque typed request before entering the worker pool. Program and destination chains verify committed bytes. | Legacy proposal lifecycle, wallet, and intent routes still build adapter arguments, and infrastructure adapters remain blocking. |
