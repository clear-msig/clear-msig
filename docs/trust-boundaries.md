...
| Execution library / CLI adapter | Refuse execution, fail after a partial step, or submit the wrong transaction attempt. | Make a destination transaction with fields different from the approved preimage. | Wallet, intent, proposal, and ClearSign HTTP paths construct bounded domain values with mutually exclusive signer contexts. CI rejects raw route argument builders. Program and destination chains verify committed bytes. | The typed boundary still lives in the CLI package, configuration introspection retains a schema-parsed compatibility adapter, and infrastructure adapters remain blocking. |
