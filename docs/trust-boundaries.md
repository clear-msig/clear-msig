...
| Backend / relayer | Censor, delay, reorder independent requests, return errors, or withhold broadcast. | Forge a user signature, change committed payload fields, lower thresholds, or execute an unapproved proposal. | Compare proposal/envelope/payload hashes on chain; any authorized participant can run the CLI. Child execution has a strict command allowlist, timeout, kill-on-drop, bounded output, and structured outcome logs. | Backend still launches the CLI as a child process; in-process execution and worker isolation remain transitional. |
| CLI process | Refuse execution, crash after a partial step, or submit the wrong transaction attempt. | Make a destination transaction with fields different from the approved preimage. | Program verifies the typed action; destination chain verifies the signature; signed MessageApproval state supports resume. | CLI and backend now share a command contract, but execution logic still needs extraction into a smaller typed core crate. |
...
2. Move CLI execution logic into a small reusable Rust core called directly by
   backend workers and CLI adapters; retire child-process orchestration.
