...
| Intent library | Extract versioned intent schemas, template identifiers, canonicalization, and test vectors into a reusable crate/library. | Shipped v1. `clear-msig-intent` owns the chain-neutral schema, validation, canonical JSON/hash, template registry, and renderer. Execution and E2E import it directly; the Solana client is only the byte adapter; frontend constants are generated from the committed registry artifact. | Rust verifies every built-in JSON file and both generated artifacts. The no-std program renderer and TypeScript renderer consume the same render vectors. CI rejects schema duplication, stale generated output, unversioned built-ins, and runtime dependencies in the schema crate. |
...
3. **Next:** Add an allowlisted device capability registry and full/compact templates;
...
the document structure and execution commitments. The independent v1 intent
library now owns off-chain schema and rendering contracts, but the program does
not yet derive every human-readable sentence from raw action fields and no
hardware-specific compact template is active in this release.
