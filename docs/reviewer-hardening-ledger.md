...
| --- | --- | --- | --- |
| Reusable Rust execution core | **Partial** | Typed proposal lifecycle and execution routes build closed domain enums with explicit dry-run/pre-signed/backend contexts, typed numeric fields, and bounded values. CI rejects raw argument vectors or flags in those modules. | Legacy proposal, wallet, and intent routes still produce adapter arguments, and the shared crate still contains blocking Solana/Ika adapters. Migrate those route constructors, then split cancellable ports into a smaller async core. |
