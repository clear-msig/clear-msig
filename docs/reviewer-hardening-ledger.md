...
| --- | --- | --- | --- |
| Reusable Rust execution core | **Partial** | Security-critical typed proposal routes build `TypedProposalExecution` values with typed numeric fields and bounded collections; CI rejects raw argument vectors or flags in that module. All backend work is validated into an opaque `ExecutionRequest` before queueing. | Legacy create/approve/cancel, wallet, and intent routes still produce adapter arguments, and the shared crate still contains blocking Solana/Ika adapters. Migrate those route constructors, then split cancellable ports into a smaller async core. |
