...
| --- | --- | --- | --- |
| Reusable Rust execution core | **Partial** | Backend validates adapter arguments into an opaque `ExecutionRequest` before queueing; bounded workers accept only that typed request through `execute_request`. Backend has no process spawn, CLI binary path, SSE spawn route, or direct `execute_args` access. CI enforces the boundary. | HTTP route builders still produce adapter arguments, and the shared crate still contains blocking Solana/Ika adapters. Replace route argv builders with typed constructors, then split cancellable ports into a smaller async core. |
