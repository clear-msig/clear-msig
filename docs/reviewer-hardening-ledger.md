...
| --- | --- | --- | --- |
| Reusable Rust execution core | **Partial** | Backend calls `clear_msig_cli::execute_args`; the CLI binary wraps the same library handlers. Backend has no process spawn, CLI binary path, or SSE spawn route. CI runs `scripts/check-backend-architecture.sh`. | The shared crate still exposes CLI-shaped arguments and blocking Solana/Ika adapters. Split typed commands into a smaller async core. |
