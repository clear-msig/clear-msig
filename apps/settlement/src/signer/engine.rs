...
use super::{bitcoin::BitcoinSigner, evm::EvmSigner, solana::SolanaSigner, zcash::ZcashSigner};
...
pub trait ChainSigner: Send + Sync {
    async fn transfer(&self, request: &AssetTransferRequest)
...
impl SignerEngine {
    ...
    pub async fn has_sufficient_balance(
        ...
        match chain_family {
            ...
            ChainFamily::Bitcoin => self.bitcoin.has_sufficient_balance(amount_minor).await,
            ChainFamily::Zcash => self.zcash.has_sufficient_balance(amount_minor).await,
        }
    }
}
