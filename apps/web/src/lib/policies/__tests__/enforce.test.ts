...
describe("policy enforcement guardrails", () => {
  ...
  it.each([
    { ticker: "ETH", decimals: 18, cap: "0.5", raw: 500_000_000_000_000_000n },
    { ticker: "BTC", decimals: 8, cap: "0.001", raw: 100_000n },
    { ticker: "ZEC", decimals: 8, cap: "0.001", raw: 100_000n },
    { ticker: "USDC", decimals: 6, cap: "1.25", raw: 1_250_000n },
    { ticker: "HYPE", decimals: 18, cap: "2", raw: 2_000_000_000_000_000_000n },
  ])(
    "encodes authoritative $ticker amount, allowlist, hours, and send-count policy",
    ({ ticker, decimals, cap, raw }) => {
      const recipient = `${ticker.toLowerCase()}-recipient`;
      const encoded = encodeTypedRemoteSendPolicy(
        {
          evaluation: null,
          rule: null,
          conditions: [
            { kind: "recipient", mode: "allowlist", addresses: [recipient] },
            { kind: "amount", ticker, maxDisplay: cap },
          ],
          extraApprovers: [],
          extraCooldownSeconds: 0,
          allowedTimeWindow: {
        ...
        { assetTicker: ticker, decimals },
      );

      expect(encoded).not.toBeNull();
      expect(encoded!.bytes[4]).toBe(1);
      expect(readU64Le(encoded!.bytes, 5)).toBe(raw);
      expect(encoded!.bytes[17]).toBe(1);
      expect(toHex(encoded!.bytes.slice(19, 51))).toBe(
        toHex(sha256(new TextEncoder().encode(recipient))),
      );
      expect(Array.from(encoded!.bytes.slice(51))).toEqual([
        2, 8, 0, 2, 0, 0, 0, 0x80, 0x51, 0x01, 0,
        3, 5, 0, 8, 18, 0b0011_1110, 60, 0,
      ]);
    },
  );
