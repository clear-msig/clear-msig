...
export function verifyTypedClearSignMessageText(
  ...
  const voteVerb = expectedVoteVerb(descriptor.action);
  const isV4 = descriptor.message_flavor === "clearsign_v4_document";
  if (
    isV4 &&
    voteVerb === "propose" &&
    !/^[0-9a-fA-F]+$/.test(descriptor.canonical_intent_hex ?? "")
  ) {
    throw new TypedClearSignMessageVerificationError(
      "ClearSign v4 proposal is missing its canonical intent bytes.",
    );
  }
  ...
  if (
    isV4 &&
    text.split("Protocol: clearsig-intent-v4@1").length - 1 !== 1
  ) {
    throw new TypedClearSignMessageVerificationError(
