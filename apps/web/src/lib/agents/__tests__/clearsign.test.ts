...
describe("agent ClearSign binding", () => {
  it("builds executor fields for typed agent approval", () => {
    ...
    expect(binding.payloadHash).toBeUndefined();
    expect(binding.envelopeHash).toBeUndefined();
    ...
    expect(binding.signableText).toBeUndefined();
  ...
  it("changes the untrusted risk input when risk fields change", () => {
