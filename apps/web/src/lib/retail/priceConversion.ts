...
const _LIVE_PRICES_USD = new Map<
  string,
  { usdPerWhole: number; observedAt: number }
>();
...
export function _setLivePrice(ticker: string, usdPerWhole: number): void {
  ...
  _LIVE_PRICES_USD.set(ticker.toUpperCase(), {
    usdPerWhole,
...
export function _snapshotLivePrices(): Record<string, number> {
  ...
  for (const [k, v] of _LIVE_PRICES_USD) out[k] = v.usdPerWhole;
  return out;
}
...
export interface PriceQuote {
  ...
  sourceId: "coingecko-api" | "static-fallback";
  observedAt: number;
}
...
export function quotePerWhole(ticker: string): PriceQuote | null {
  ...
  if (live) {
    return {
      usdPerWhole: live.usdPerWhole,
      source: "live",
      sourceId: "coingecko-api",
  ...
  return {
    usdPerWhole: fallback,
    source: "fallback",
    sourceId: "static-fallback",
    observedAt: 0,
  };
}
