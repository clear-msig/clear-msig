...
import { currentSessionVenue, sessionAllowsVenue } from "@/features/agents/domain/library";
import { AGENT_TRACK_RECORD_SOURCES, type AgentAllocationRecommendation, type AgentExecutionRecord, type AgentLeaderboardEntry, type AgentLibraryMetrics, type AgentMarketDataSnapshot, type AgentProfile, type AgentScorecard, type AgentSessionGrant, type AgentTrackRecordBook, type AgentTrackRecordSource, type AgentTradeProposal, type AgentVaultPolicy, CLEARSIG_TRADER_LIBRARY, type ClearSigTraderRisk, type ClearSigTraderTemplate, estimateAgentOpenTradePerformance, isAgentSessionCurrent } from "@/features/agents/domain/runtime";
...
export function AgentLibraryScreen({ controller }: { controller: ReturnType<typeof useAgentLibraryController> }) {
  const {
    addDemoHistory,
    chooseTrader,
    chosen,
        ...
        {recipes.map((template) => {
          ...
          return (
            <button
              ...
              className={clsx(
                "group flex min-h-[12rem] flex-col rounded-card bg-canvas p-4 text-left",
...
function TraderCard({
  ...
  return (
    <article className="flex min-h-[25rem] flex-col rounded-card bg-surface-raised p-5 shadow-card-rest">
...
function TrackedAgentCard({
  ...
  return (
    <article className="flex flex-col rounded-card bg-surface-raised p-5 shadow-card-rest">
      <div className="flex items-start justify-between gap-3">
