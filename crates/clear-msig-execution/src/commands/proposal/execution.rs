...
#[derive(Clone, Copy)]
pub(super) enum IkaOnchainSignMode {
    LegacyProposal,
    TypedChainSend {
...
pub(super) fn message_approval_is_signed(data: &[u8]) -> bool {
...
pub(super) fn attestation_session_for_binding(
...
pub(super) fn build_broadcast_inputs(
    chain_kind: u8,
...
pub(super) fn parse_hex_local(s: &str) -> Result<Vec<u8>> {
...
pub(super) fn decode_hex_32(value: &str, field: &str) -> Result<[u8; 32]> {
...
pub(super) fn ensure_typed_text(value: &str, field: &str) -> Result<()> {
...
pub(super) fn typed_vote_message(
...
pub(super) fn approval_requirement_label(kind: ClearSignVoteKind, required: u8) -> &'static [u8] {
    match (kind, required) {
...
pub(super) fn typed_message_flavor(clear_text: &[u8]) -> &'static str {
...
pub(super) fn resolve_approved_typed_proposal(
...
pub(super) fn ensure_typed_action(
...
pub(super) fn ensure_generic_typed_execute_allowed(
    proposal: &accounts::TypedProposalAccount,
) -> Result<()> {
...
pub(super) fn committed_governance_payload(
    committed: &[u8],
...
pub(super) fn parse_return_row(row: &str) -> Result<(Pubkey, u64)> {
    parse_recipient_lamports_row(row, "return")
}

pub(super) fn parse_token_return_row(row: &str) -> Result<(Pubkey, Pubkey, u64)> {
...
pub(super) fn intent_tx_template_hash(intent: &accounts::IntentAccount) -> Result<[u8; 32]> {
...
pub(super) fn intent_policy_ciphertexts_hash(intent: &accounts::IntentAccount) -> Result<[u8; 32]> {
...
pub(super) fn parse_recipient_lamports_row(row: &str, label: &str) -> Result<(Pubkey, u64)> {
...
pub(super) fn vault_pubkey(wallet_pubkey: Pubkey) -> Pubkey {
...
pub(super) fn policy_spend_pubkey(wallet_pubkey: Pubkey, intent_pubkey: Pubkey) -> Pubkey {
...
pub(super) fn member_allowance_pubkey(wallet_pubkey: Pubkey, intent_pubkey: Pubkey) -> Pubkey {
...
pub(super) fn agent_session_pubkey(wallet_pubkey: Pubkey, session_id_hash: [u8; 32]) -> Pubkey {
...
pub(super) fn wallet_policy_pubkey(wallet_pubkey: Pubkey) -> Pubkey {
...
pub(super) fn typed_approve_or_cancel(
    config: &RuntimeConfig,
    wallet_name: &str,
    proposal_addr_str: &str,
    is_approve: bool,
...
pub(super) fn approve_or_cancel(
    config: &RuntimeConfig,
    wallet_name: &str,
    proposal_addr_str: &str,
