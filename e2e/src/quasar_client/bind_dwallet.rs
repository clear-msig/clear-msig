//! Mirror of the Quasar-generated BindDwalletInstruction at
//! `programs/clear-wallet/target/client/rust/clear-wallet-client/src/instructions/bind_dwallet.rs`.
//!
//! Includes the `dwallet_ownership` account that the program requires
//! (added when the per-dWallet ownership-lock PDA was introduced). The
...
impl From<BindDwalletInstruction> for Instruction {
    fn from(ix: BindDwalletInstruction) -> Instruction {
        let accounts = vec![
            ...
            AccountMeta::new_readonly(ix.instructions_sysvar, false),
        ];
