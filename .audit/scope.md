...
- **Backend API #1** (`apps/api/`): Rust/Axum. Links `clear-msig-execution` and `clear-msig-command-contract` crates in-process. Serves ClearSign proposals/intents/pro tiers.
- **Backend API #2** (`apps/settlement/`): Rust/Axum, Postgres, Paystack/Kora fiat rails, treasury signer, background workers. Separate service, same Dockerfile.
- **Frontend** (`apps/web/`): Next.js 15 + React 19, TS. `@dynamic-labs/*` (embedded/WaaS), `@ledgerhq/hw-app-solana`, `@solana/web3.js`, `tweetnacl`, `bs58`, `@noble/*`, `@encrypt.xyz/pre-alpha-solana-client` (new pre-alpha dep). Mobile wrapper dirs (android/ios).
- **CLI** (`apps/cli/`), **e2e** (`apps/e2e/`), shared crates `crates/clear-msig-command-contract`, `crates/clear-msig-execution`, vendored `deps/solana-curve25519`.
- Lockfiles committed: root `Cargo.lock`, `apps/settlement/Cargo.lock`, `apps/web/package-lock.json`.

## Entrypoints
| Path | Type | Untrusted input? |
|---|---|---|
| `apps/api/src/{clearsign,intents,proposals,pro,wallet}.rs` | Axum handlers | Yes — proposal/intent submission, ClearSign envelopes |
| `apps/api/src/runner.rs`, `runtime.rs` | Execution runner for on-chain instructions | Indirect |
| `apps/settlement/src/http/handlers.rs` | Axum routes, settlement/fiat | Yes |
| `apps/settlement/src/kora/{client,events,signature}.rs` | Kora webhook ingestion | Yes — external webhook |
| `apps/settlement/src/paystack/{client,events,signature}.rs` | Paystack webhook ingestion | Yes — external webhook |
| `apps/settlement/src/workers/*.rs` | Background pollers (chain_confirmation, disbursement, payout_dispatch, webhook_processing) | Processes previously-ingested data async |
| `apps/web/src/app/api/*` | Next.js API routes | Yes |
| `apps/web/src/app/app/**/page.tsx` | Client tx-construction pages | Yes |
| `programs/clear-wallet/src/instructions/*` | On-chain instruction handlers | Yes — final trust boundary |
| `apps/cli/src` | CLI entrypoint | Operator |

## Trust Boundaries
| From | To | Crossing point | Validation? |
|---|---|---|---|
| Public internet | backend-api | `apps/api/src/validation.rs`, `cors.rs`, `rate_limit.rs` | Explicit `ensure_*` validators, rate limiter, CORS allowlist |
...
| App → Dynamic Labs/Ledger SDKs | Third-party wallet infra | `apps/web/src/lib/hooks/use*` | Client-side trust in vendor SDK |

## Integrated Services
| Service | Access layer | Credential source |
|---|---|---|
| Postgres | `apps/settlement/src/db.rs`, `migrations/` | `DATABASE_URL` |
...
| Kora | `apps/settlement/src/kora/*` | `CLEAR_MSIG_DEFAULT_DWALLET_PROGRAM`, `CLEAR_MSIG_DEFAULT_GRPC_URL` (sync:false) |
...
## Agentic Surface
- No MCP server/client/LLM-agent-loop code found. "Agent" in this repo = **trading-agent product feature** (autonomous trading vault sessions): `AgentTradeApproval`, `AgentSessionGrant`, `AgentRiskPolicy`, `AgentTradeSettlement` in `clearsign.rs`; `docs/agent-trading-vault*.md`, `docs/agent-vault-security.md`, `apps/web/src/app/agent(s)`, `examples/agent-signal-runner`, `examples/hyperliquid-testnet-executor`.
...
1. `apps/settlement/src/signer/solana.rs` (+`signer/engine.rs`) — custodial hot-wallet signing outside multisig policy; modified this update.
...
7. `apps/settlement/src/paystack/signature.rs`, `kora/signature.rs` — webhook signature verification.
8. `apps/settlement/src/workers/{disbursement,payout_dispatch}.rs` — automated fund movement; check idempotency/replay handling.
9. `render.yaml` — expanded secret surface; verify no plaintext defaults.
10. `apps/api/src/validation.rs`, `rate_limit.rs`, `cors.rs` — primary input-sanitization layer.
