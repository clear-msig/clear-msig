...
- **Confirmed concrete example:** `apps/web/node_modules/bigint-buffer/package.json` has an explicit `"install": "npm run rebuild || echo ..."` → `node-gyp rebuild` → compiles `binding.gyp` at install time — a real `install` script, not just implicit gyp auto-build. `bufferutil`, `utf-8-validate`, `sharp` also ship `binding.gyp`.
- **Verdict:** [HIGH] Lifecycle scripts unrestricted in CI and local installs — single highest-leverage supply-chain gap, independent of whether any currently-resolved package is malicious today. Fix: `apps/web/.npmrc` with `ignore-scripts=true`, CI `npm ci --ignore-scripts`, explicit allowlisted rebuild step for packages that need native builds.
...
- **Fix:** file upstream issue against `@dynamic-labs-wallet` to collapse to one `core` version; in the interim use `npm overrides` in `apps/web/package.json` to force a single patched axios resolution, verify wallet SDK still functions.
...
- Rust: 5 committed `Cargo.lock` files (root 821-823, rust-settlement 819-830, programs/clear-wallet 292, client 71, deps/solana-curve25519 28) — `apps/settlement/Cargo.toml` diff for this update is exactly one new dependency: `solana-system-interface = { version = "3", features = ["bincode"] }` — official Solana Labs crate, not a typosquat, no concern.
- npm: `apps/web/package-lock.json` ~1005-1011 packages, all from registry.npmjs.org, full integrity-hash coverage, no swapped registry detected. All direct deps use `^` floating ranges including crypto-critical ones and the pre-alpha `@encrypt.xyz` package — recommend exact-pinning the latter given its trust profile.
...
- Working tree (`--no-git`): 62 findings, all inside gitignored `apps/web/.next/` build cache (auto-generated, not committed).
- **Verdict: clean.** No real committed secrets.

## Coverage
- **Completed:** manual lockfile/manifest/workflow inspection, `npm ls axios` (confirmed reachability), gitleaks full-history + working-tree scan, live `npm view` against `@encrypt.xyz`/`@ika.xyz` registry metadata, `apps/settlement/Cargo.toml` diff review.
