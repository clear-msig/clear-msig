...
pub async fn run_webhook_processing_pass(pool: &PgPool) -> anyhow::Result<u64> {
    ...
    if !events.is_empty() {
        info!(queued = events.len(), "Webhook processing worker picked inbox events");
    }
                    ...
                    .await
                    {
                        Ok(result) => {
                            if result.rows_affected() == 0 {
                                ...
                                processing_error = Some("transfer_success_intent_not_found".to_string());
                            }
                        }
                        Err(err) => {
                            error!(%event_id, reference = %reference, error = %err, "Failed updating intent for transfer.success");
                            processing_error = Some(format!("transfer_success_intent_update_failed: {err}"));
                        }
                    }
                }
            }
                    ...
                    .await
                    {
                        Ok(result) => {
                            if result.rows_affected() == 0 {
                                ...
                                processing_error = Some("transfer_failure_intent_not_found".to_string());
                            }
                        }
                        Err(err) => {
                ...
                .await
                {
                    Ok(result) => {
                        if result.rows_affected() == 0 {
                            ...
                            processing_error = Some("charge_success_reference_not_found".to_string());
                        }
                    }
                    Err(err) => {
