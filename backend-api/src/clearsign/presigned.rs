...
impl PreSigned {
    pub(crate) fn ensure_valid(&self) -> Result<(), ApiError> {
        ...
        if let Some(flavor) = &self.message_flavor {
            match flavor.as_str() {
                "offchain_v1" | "plain_v2" | "clearsign_v2_vote_hash" | "clearsign_v2_text" => {}
                other => {
                    return Err(ApiError::BadRequest(format!(
                        "message_flavor must be offchain_v1, plain_v2, clearsign_v2_vote_hash, or clearsign_v2_text, got {other}"
                    )));
                }
            }
        }
