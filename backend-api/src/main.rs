...
fn normalize_escrow_policy(policy: ProEscrowPolicy) -> Result<ProEscrowPolicy, ApiError> {
    ...
    if enforcement != "approval_workflow" && enforcement != "onchain_policy_pending" {
        return Err(ApiError::BadRequest(
            "unsupported escrow policy enforcement".to_string(),
        ));
    }
    ensure_non_empty(commitment, "policy.commitment")?;
    Ok(ProEscrowPolicy {
        version: 1,
        mode: mode.to_string(),
...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/v1/pro/wallets/{name}/escrows",
            get(list_pro_escrows).post(upsert_pro_escrow),
        )
        .route(
            "/v1/pro/wallets/{name}/escrows/delete",
            post(delete_pro_escrow),
        )
        .route(
