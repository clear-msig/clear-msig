...
pub fn build_preimage(
    ...
    let mut p = 0;
    out[p] = 0x00;
    p += 1; // op = Solana transfer
    out[p..p + 32].copy_from_slice(&destination);
    p += 32;
    out[p..p + 8].copy_from_slice(&amount.to_le_bytes());
    p += 8;
    out[p..p + 32].copy_from_slice(&nonce_value);
    p += 32;
...
pub fn build_tx_message(
    ...
    // Header
    out[p] = 1;
    p += 1; // num_required_signatures
    out[p] = 0;
    p += 1; // num_readonly_signed
    out[p] = 2;
    p += 1; // num_readonly_unsigned (sysvar + system_program)

    // Account keys (5)
    out[p] = 5;
    p += 1;
    out[p..p + 32].copy_from_slice(signer_pubkey);
    p += 32;
    out[p..p + 32].copy_from_slice(nonce_account);
    p += 32;
    out[p..p + 32].copy_from_slice(&destination);
    p += 32;
    out[p..p + 32].copy_from_slice(&SYSVAR_RECENT_BLOCKHASHES);
    p += 32;
    out[p..p + 32].copy_from_slice(&SYSTEM_PROGRAM);
    p += 32;
    ...
    out[p] = 4;
    p += 1; // program_id_index = system_program
    out[p] = 3;
    p += 1; // accounts len
    out[p] = 1;
    p += 1; // nonce_account
    ...
    p += 1; // sysvar
    out[p] = 0;
    p += 1; // authority (signer)
    out[p] = 4;
    p += 1; // data len
    out[p..p + 4].copy_from_slice(&[4, 0, 0, 0]);
    p += 4; // AdvanceNonceAccount
    ...
    out[p] = 12;
    p += 1; // data len
    out[p..p + 4].copy_from_slice(&2u32.to_le_bytes());
    p += 4; // Transfer instruction
    out[p..p + 8].copy_from_slice(&amount.to_le_bytes());
    p += 8;
